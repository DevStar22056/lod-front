  ...
  actions: {
    ...
    async doExport({ commit, getters }) {
      try {
        ...
        const response = await IamService.fetchUsers(filter, getters.orderBy, null, null);
    ...
    async doDisableAllSelected({ getters, commit, dispatch }) {
      try {
        ...
        await IamService.disable(selectedRows.map((user) => user.id));
    ...
    async doEnableAllSelected({ commit, getters, dispatch }) {
      try {
        ...
        await IamService.enable(selectedRows.map((user) => user.id));
    ...
    async doRemoveAllSelected({ commit, getters, dispatch }) {
      try {
        ...
        await IamService.remove(selectedRows.map((row) => row.email), [], true);
