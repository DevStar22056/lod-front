...
import subscriptionPaymentListExporterFields from '@/modules/subscription-payment/subscription-payment-list-exporter-fields';
...
export default {
  ...
  actions: {
    ...
    async doExport({ commit, getters }) {
      try {
        if (
          !subscriptionPaymentListExporterFields ||
          !subscriptionPaymentListExporterFields.length
        ) {
          throw new Error('subscriptionPaymentListExporterFields is required');
        }
        ...
        const response = await SubscriptionPaymentService.list(filter, getters.orderBy, null, null);
    ...
    async doFetch({ commit, getters }, { filter, keepPagination } = {}) {
      try {
        ...
        const response = await SubscriptionPaymentService.list(
          filter,
          getters.orderBy,
          getters.limit,
          getters.offset,
