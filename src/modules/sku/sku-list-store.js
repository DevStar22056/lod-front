...
import skuListExporterFields from '@/modules/sku/sku-list-exporter-fields';
...
export default {
  ...
  actions: {
    ...
    async doExport({ commit, getters }) {
      try {
        if (!skuListExporterFields || !skuListExporterFields.length) {
          throw new Error('skuListExporterFields is required');
        }
        ...
        const response = await SkuService.list(filter, getters.orderBy, null, null);

        new Exporter(skuListExporterFields, 'sku').transformAndExportAsExcelFile(response.rows);
