...
import shipmentListExporterFields from '@/modules/shipment/shipment-list-exporter-fields';
...
export default {
  ...
  actions: {
    ...
    async doExport({ commit, getters }) {
      try {
        if (!shipmentListExporterFields || !shipmentListExporterFields.length) {
          throw new Error('shipmentListExporterFields is required');
        }
        ...
        const response = await ShipmentService.list(filter, getters.orderBy, null, null);

        new Exporter(shipmentListExporterFields, 'shipment').transformAndExportAsExcelFile(
          response.rows,
        );
