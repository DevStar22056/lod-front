...
export default {
  ...
  actions: {
    ...
    async doExport({ commit, getters }) {
      try {
        if (!memberListExporterFields || !memberListExporterFields.length) {
          throw new Error('memberListExporterFields is required');
        }
        ...
        const response = await MemberService.list(filter, getters.orderBy, null, null);

        new Exporter(memberListExporterFields, 'member').transformAndExportAsExcelFile(response.rows);
    ...
    async doFetch({ commit, getters }, { filter, keepPagination } = {}) {
      try {
        ...
        const response = await MemberService.list(filter, getters.orderBy, getters.limit, getters.offset);
